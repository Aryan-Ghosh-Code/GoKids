  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        ...
        {/* Action Controls (Sort & Filter) */}
        <div className="flex gap-3 w-full sm:w-auto">
          {/* Custom Sort Dropdown */}
          <div
            className="relative flex-1 sm:flex-none"
            ref={(el) => {
              if (el !== activeRef) setActiveRef(el);
            }}
          >
            <button
              onClick={() => setSortOpen(!sortOpen)}
              className="w-full flex items-center justify-between pl-5 pr-4 py-3 rounded-full text-sm font-bold bg-white outline-none cursor-pointer transition-all hover:bg-[#F9FAFB] active:scale-95"
              style={{
                border: "1.5px solid #E5E7EB",
                color: "#1A1A1A",
                fontFamily: "var(--font-nunito)",
            ...
            >
              <span className="flex items-center gap-2">
                <span className="text-brand-grey-text font-medium">Sort:</span>
            ...
            <AnimatePresence>
              {sortOpen && (
                <motion.div
                  ...
                  className="absolute left-0 right-0 mt-2 py-1.5 rounded-2xl bg-white border border-[#E5E7EB] shadow-lg z-30 overflow-hidden sm:left-auto sm:right-0 sm:w-48"
                >
                  {SORT_OPTIONS.map((o) => {
          ...
          <button
            ...
            className="lg:hidden flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-3 rounded-full text-sm font-bold transition-all"
            style={{
              background: activeFiltersCount > 0 ? "#F5C518" : "white",
              color: "#1A1A1A",
              border:
                "1.5px solid " +
                (activeFiltersCount > 0 ? "#F5C518" : "#E5E7EB"),
              fontFamily: "var(--font-nunito)",
            }}
          >
            ...
            {activeFiltersCount > 0 && (
              <span
                className="bg-white rounded-full px-1.5 text-xs font-extrabold"
                style={{ color: "#1A1A1A" }}
              >
                {activeFiltersCount}
              </span>
